...
| E-110 | **Beim Antworten und Weiterleiten kommen nicht alle Bilder ins Verfassenfenster** — ein Teil steht als **grauer Kasten** da | **offen, Ursache NICHT gemessen.** Von Gregor am 18.09.2026 an **1.0.72** gemeldet, mit drei Bildschirmfotos: *„beim weiterleiten und antworten werden nicht alle bilder so übernommen. sollte doch schon gefixt sein?"* (Titelzeile seiner Fassung: `Eudora 7.2.0.72 / Paket 1.0.72+e2f2d07 2026-09-17 21:56`; Nachricht `Re: FW: Durchbrechen Sie die Eskalations-Spirale`). **Was belegt ist:** im **Lesefenster** derselben Nachricht sind **alle** Bilder vollständig da; im **Verfassenfenster** der Antwort sind mehrere graue Flächen, während andere durchkommen — das FreieWelt-Banner, das Fernsehbild und der schwarze *FALTBLATT*-Kasten. **KEIN Rückschritt, und das ist der wichtigste Satz dieses Eintrags:** die grauen Kästen haben die **richtige Größe**, und der Text liegt **nicht** mehr darüber. **E-103 und E-106 arbeiten also** — dies ist eine andere Schicht als die Größen- und Überlappungsbefunde, und wer das übersieht, sucht am falschen Ende. **Aus dem eigenen Bestand gedeutet, nicht neu erfunden:** `CHANGELOG.md`, Abschnitt 7.2.0.58, sagt bereits *„Den grauen Kasten selbst gibt es ohnehin: **Paige zeichnet ihn für jedes Bild, das es nicht geladen hat** — auf Gregors Bildern stehen welche in **echten** Bildmaßen."* Ein grauer Kasten in richtiger Größe heißt demnach: **das Bild ist nicht geladen worden**, die Maßangabe war aber bekannt. **Abzugrenzen vom grauen Kasten aus 7.2.0.57:** der stammte aus der 200×90-Vorgabe für Bilder **ohne** bekanntes Maß und ist in 7.2.0.58 ersatzlos zurückgenommen worden — ein anderer Kasten aus einer anderen Ursache. **VERDACHT, ausdrücklich UNGEMESSEN** (`Arbeitsweise/pruefen-statt-vermuten.md`): die Trennlinie könnte zwischen Bildern verlaufen, die **in der Nachricht** liegen, und solchen, die **aus dem Netz** nachgeladen werden müssten. Die Gabelung steht in `PgEmbeddedImage.cpp:362-415`, am Quelltext nachgeschlagen: erst der MIME-Speicher der Nachricht (`pMsg->GetPartAsFile(srcURI, …)`, `:382`), und **nur wenn `filePath` leer bleibt** (`:387`) der Netzabruf `fetch_url_schmookie` (`:401`) — Letzterer aber **nur bei `bAllowThreadedFetch`** (`:389`); sonst `fRet = true` mit QUALCOMMs Kommentar *„we **do** want to keep on trying (so that we get called later when we will allow the threaded fetch)"* (`:407-414`). **Ungemessen ist, ob dieses „später" beim Antworten überhaupt eintritt.** `eCallback` ruft `PgLoadUrlImage` zweimal: bei `EMBED_PREPARE_IMAGE` mit `false` (`:268`) und bei `EMBED_LOAD_IMAGE` mit `true` (`:280`) — kommt der zweite Aufruf im Verfassenfenster nicht, bleibt jedes Netzbild grau. **Eine zweite, ebenso ungemessene Gabelung am selben Ort, die beim Messen mitgeprüft gehört:** der MIME-Speicher-Weg hängt an `pSB->pMessage` (`:376-377`, `if (pMsg)`); gesetzt wird das in `PgMsgView::SetMessageStuff` (`PgMsgView.cpp:181-182`) auf `&pMsgDoc->m_QCMessage`. Bei einer **Antwort** entsteht das zitierte HTML aber neu über `QuoteText`, und die `cid:`-Verweise zeigen auf den MIME-Speicher der **Original**nachricht — ob die Verfassen-`QCMessage` die überhaupt kennt, ist offen. `PgStuffBucket::kMessage` deckt laut Kommentar *„comp, read, preview"* (`PgStuffBucket.h:21`), das Verfassenfenster fällt also **nicht** schon an der Abfrage in `:362` heraus; entschieden wird es an `pMessage`. **GEMESSEN AM PROTOKOLL, 18.09.2026** — `C:\Users\Gregor\Eudora72-1.0.72-release\Mailverzeichnis\eudora.log`, 150.580 Byte, 08:20, letzte Zeile `Logging shutdown`, **keine `Exception.log`**; E-108 hält also auch in diesem Lauf. Gezählt: **128 × `E-95 Bild`**, **32 × `E-106 groesser als angegeben`**, **11 × `E-103 Nachtrag`**, **11 × `E-103 Neuzeichnen`**. **Der E-106-Block läuft bei ihm 32-mal an** — bei mir null (L-15.9); das bestätigt ein zweites Mal, dass dem Prüfstand nur das Testdatum fehlt. **Und hier ist der Ausreißer, der zählt** (`Arbeitsweise/ausreisser-ist-der-befund.md`): `attr` und `embed` sind **nicht** durchgehend gleich. Von 128 Zeilen tragen **76** gleiche Maße — und **52** haben **`embed=0x0`**, bei durchweg bekannter Breite: `attr=56x0` (20×), `attr=110x0` (10×), `attr=30x0` (9×), `attr=150x0` (5×), `attr=79x0` (3×), `attr=183x0` (3×), `attr=194x0`, `attr=120x0`. **`embed=0x0` heißt: für dieses Bild liegt kein Embed vor.** Bei allen 52 ist die Höhe zugleich 0 (`attr=NNNx0`) — die Maße kommen also aus dem HTML-Attribut, nicht aus einer geladenen Datei. **Damit ist belegt, dass der Platz stimmt und die Bildpunkte fehlen**, und zwar an einer nachzählbaren Menge statt an einer Beispielzeile: **E-110 liegt hinter dem Layout, nicht darin.** Wer hier bei E-103 oder E-106 sucht, sucht am Fall vorbei. **EINE FALSCHE SPUR, ausdrücklich festgehalten, damit sie niemand ein zweites Mal geht:** im Protokoll ist **jede** der 128 `src=`-Angaben **exakt 32 Zeichen** lang (`https://004.frnl.de/admin/images`, `https://gallery.mailchimp.com/70`) — das sieht zwingend nach einem abgeschnittenen URL-Feld und damit nach der Ursache aus. **Es ist die Spurmarke selbst:** `PGHTMIMP.CPP:2250` kopiert mit `for (i = 0; i < 32 …)`, und `:2255` formatiert zusätzlich mit `%.32s`. **Anzeigefehler der eigenen Messung, kein Befund** (`Arbeitsweise/anzeige-ist-kein-zustand.md`). **OFFENER PUNKT — die Spurmarke taugt für diesen Befund nicht, und das ist vor der nächsten Messung zu beheben:** (a) 32 Zeichen reichen bei diesen Adressen nicht einmal bis zum Dateinamen, man sieht also nicht, **welches** Bild grau bleibt; (b) `attr` ist `image_record.source_width/height` und `embed` ist `mess_ptr->width/height` (`PGHTMIMP.CPP:2258-2259`) — **beides sind Maße**, keiner der Werte sagt, ob **Bilddaten** dahinterliegen oder nur ein Platzhalter. Gebraucht wird `image_data` neben dem Maß, in **derselben** Zeile (`Arbeitsweise/zwei-werte-in-eine-ausgabe.md`). **Ursache weiterhin NICHT benannt** — Gregor am 18.09.2026: *„aber erstmal das andere zu ende führen, sonst kommst du durcheinander."* **WAS DIE MESSUNG NOCH NICHT ENTSCHEIDET:** Gregors `eudora.log` zu diesem Lauf — 1.0.72 bringt `LogLevel=58527` mit, und je Bild steht dort eine `E-95 Bild`-Zeile mit `attr`, `embed` und `ascent`. **Daran ist abzulesen, ob der Abruf gar nicht erst versucht wird oder ob er scheitert**, und ob die grauen Bilder `embed=0x0` tragen. Erst danach wird eine Ursache benannt. **Kein Datenverlust** — das Lesefenster zeigt die Nachricht vollständig; offen ist, was beim Empfänger der Antwort ankommt, und **auch das ist nicht gemessen**. **Offener Punkt dazu (E-109, Regressionstests):** seit L-15.9 ist belegt, dass der Prüfstand Bilder **lädt** — acht `E-95 Bild`-Zeilen im Messlauf. Damit ist dieser Fall im Gegensatz zu E-106 **auf dem Prüfstand nachstellbar**: eine Nachricht mit einem `cid:`-Bild und einem Netzbild öffnen, beantworten und die `E-95 Bild`-Zeilen beider Fenster vergleichen. Gregor am 17.09.2026: *„für all diese szenarien brauchen wir regression tests!"* |
