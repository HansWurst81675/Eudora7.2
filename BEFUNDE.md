...
| E-88 | **beim Weiterleiten und Antworten geht das Seitenlayout verloren** — Kästen, Hintergründe und Bildgrößen fehlen, Emoji werden zu Fragezeichen; der Empfänger bekommt eine Nachricht, die anders aussieht als die gelesene | **in Arbeit**, 14.09.2026. **Nicht dasselbe wie E-87:** dort ging *jede* Auszeichnung verloren, das ist mit `KeepHTMLInResponses` behoben — Fettung, Kursiv, Verweise, Schriftfarben und `<table bgcolor>` überleben jetzt. Was bleibt, ist die **Grenze des Editors**: verfasst wird mit Paige, und dessen HTML-Leser kennt **keine einzige CSS-Eigenschaft** (`PGHTMDEF.C:20-48`). **Gemessen an Gregors Postfach:** von 1506 `<img>` tragen 1262 die Größe als HTML-Attribut (Paige kann sie, `PGHTMIMP.CPP:1897-1916`), **162 nur als CSS** — die erscheinen in Originalgröße und überlagern den Text. Ein zweiter, HTML-fähiger Editor existiert nicht: die Trident-Ansichten gibt es nur zum Lesen, `CCompMessageFrame` erzeugt fest `PgCompMsgView`. **Betrifft POP3 genauso wie IMAP** — `ComposeMessage` und `QuoteText` arbeiten auf der lokalen Mailbox, nicht auf der Verbindung. **Gregors Entscheidung vom 14.09.2026:** das Original-HTML unangetastet aufheben und beim Senden aus Original + Zusatz zusammensetzen; der Editor zeigt nur eine Textfassung. Sein Wort: *„ich nehme deine empfehlung."* **Zwei Sicherungen sind Bedingung:** eine Spurmarke, die vor jedem Absenden protokolliert, welche Fassung genommen wurde, und ein INI-Schalter zum Abschalten |
