...
| E-26 | der Absturzbericht nennt die Ladeadresse jedes Moduls (`QCExceptionHandler::WriteModuleTable`) | **behoben** (`277d3a4`) — kein eigener Abschnitt, beschrieben in `Befunde/SPUR.md` unter *„Das Absturzprotokoll"* |
...
| E-76 | das **freischwebende Filterfenster** lässt sich nur **seitlich** vergrößern, nicht nach unten | **offen**, von Gregor am 10.09.2026 an 1.0.40 gemeldet: *„filter fenster läßt sich nicht nach unten vergrößern, nur zur seite“*, mit Bild des Fensters `WazooBar2`. **Vor dem Messen geprüft:** die Wazoo-Leiste trägt `CBRS_SIZE_DYNAMIC` (`WazooBarMgr.cpp:136`), die dynamische Größe ist also erlaubt; `FloatControlBarInMDIChild` ist im Shim **nicht** umgesetzt (`OTShim.cpp:372`), das Fenster ist demnach ein echtes schwebendes und kein MDI-Kind; und der `IsFloating`-Zweig in `SECControlBar::CalcDynamicLayout` sieht richtig aus — `LM_LENGTHY` setzt `cy`, sonst `cx`. Damit ist die Frage offen. **Spurmarke `E-76` seit 7.2.0.41:** sie nennt `nLength`, `dwMode` mit ausgeschriebenen Flags, `IsFloating` und die Maße vorher wie nachher in **einer** Zeile und schreibt nur, solange die Leiste schwebt |
