...
| E-83 | eine **IMAP-Aufgabe bleibt in der Warteschlange stehen** und wird nie gestartet — *„Waiting in the task queue to be started …"* | **offen**, von Gregor am 11.09.2026 an 1.0.48 gemeldet, mit Bildschirmfoto: eine Aufgabe *Resyncing* steht in der Liste, und beim Beenden warnt Eudora *„You currently have 1 task(s) running"*. Kennung am 11.09.2026 von ihm vergeben. **Nicht die Zertifikatsprüfung** — im selben Lauf stand die IMAP-Verbindung und eine Mail kam an. **Drei Ursachen am Quelltext ausgeschlossen:** `CanScheduleTask` blockiert nur POP-Empfang derselben Persönlichkeit (`QCTaskManager.cpp:383-390`); die verzögerte Einreihung scheidet aus, weil `DelayTasks` und `StartTasks` **niemand aufruft** (beide tot); `MaxConcurrentTasks` steht auf 10 und ist bei einer Aufgabe nicht erreicht. **Offener Verdacht:** `StartWorkerThread` prüft `pTaskInfo->m_pThread` auf NULL und tut bei NULL **nichts** — kein Start, kein Fehler, keine Meldung (`QCTaskManager.cpp:406-410`); darüber steht ein `ASSERT`, das im Freigabebau nichts tut. **Zu belegen mit einer Spurmarke**, die Zustand, `m_pThread`, aktive Aufgaben und Obergrenze in **einer** Zeile nennt |
...
| E-86 | eine **Newsletter-Mail steht in Eudora auf weißem Grund, das Logo in einem Kasten mit blauem Rahmen** — im Webbrowser dagegen alles auf schwarzem Grund und das Logo ohne Rahmen | **offen; der erste Verdacht ist WIDERLEGT**, von Gregor am 14.09.2026 an 1.0.52 gemeldet. **Der Verdacht war:** `CTridentView::WriteTempFile` (`TridentView.cpp:1282-1481`) wickelt die Mail in ein eigenes HTML-Dokument (Zeichensatzzeile, `IDS_INI_READMESSAGE_STYLE_SHEET`, Kopfzeilen, `<div>`), die Mail bringt aber ihr **eigenes** vollständiges Dokument mit `<html><head><style>…</style></head><body>` mit — MSHTML müsse bei einem zweiten `<head>`/`<body>` deren Inhalt verwerfen, womit Hintergrundfarbe **und** `img{border:0}` wegfielen: beide Symptome aus einer Ursache. **Gemessen statt geschätzt, und es stimmt nicht.** `tools/e86-fassung-bauen.pl` nimmt eine **echte** Nachricht aus `WaipuTV.mbx` und bildet den Weg des Programms nach (`AddBodyPart` nimmt den Bereich zwischen `<x-html>` und `</x-html>`, `msgutils.cpp:2365`; `AddToBody` entfernt bei `RunHtmlCode=0` — dem Vorgabewert, `EudoraRes.rc:8637` — nur `script`, `applet`, `object`, `embed`, `marquee`, **nicht** `html`/`head`/`body`, weil `bStripDocumentLevelTags` nur bei **mehreren** Nachrichten gesetzt wird, `TridentView.cpp:1409`); `tools/e86-messen.ps1` lädt beide Fassungen in MSHTML und liest den **berechneten** Stil aus dem DOM. Ergebnis für **alle fünf** Nachrichten des Postfachs, mit Bildern und mit Platzhaltern statt Bildern: `Stylesheets=3` (Eudoras **und** die beiden der Mail), `body-Hintergrund=#000000`, `img-in-a-Rahmen=0px`. Das Abbild zeigt schwarzen Grund und rahmenlose Bilder. **Der zweite `<head>`/`<body>` kostet den Hintergrund also nicht.** **Dazu passt, dass das Original den Fall längst kennt:** `CTridentView::FixupSource` (`TridentView.cpp:1912`) existiert ausdrücklich für das zweite `<body>`-Tag und kopiert dessen Attribute (`background`, `bgColor`, `text`, `link`, Ränder) auf das erste. **Die zweite Ansicht scheidet aus:** `UsingTrident()` hängt an `UseBidentAlways`, Vorgabe **1** (`EudoraRes.rc:8123`), Gregors `Eudora.ini` setzt den Schlüssel nicht, und sein Protokoll vom 14.09.2026 enthält die Spurmarke `E-80 trident:` — angezeigt wird über MSHTML, nicht über Paige. **Was im Protokoll auffällt:** jeder Aufbau meldet `konzentriert=1`; der Content Concentrator gibt einen **styled body** laut `ConConMessage.cpp:566-612` aber unverändert in Gänze aus. **Nächster Schritt, im Quellstand eingebaut (noch kein Paket gebaut, Stand 7.2.0.52):** zwei Spurmarken. `LoadMessage` legt die fertige Anzeigedatei bei eingeschaltetem Protokoll als **`E86-Anzeige.htm`** im Eudora-Verzeichnis ab — bisher löschte der nächste Aufbau sie (`unlink`), sie war nicht zu greifen; und `FixupSource` meldet in **einer** Zeile, ob es überhaupt läuft, wie viele `BODY`-Elemente MSHTML angelegt hat und welchen Hintergrund es für den angezeigten `body` **berechnet** hat (`IHTMLCurrentStyle`). Damit entscheidet der nächste Lauf den Fall, ohne dass noch einmal etwas nachgebaut werden muss |
