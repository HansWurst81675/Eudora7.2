...
| E-67 | der Trennbalken der **rechten** Leiste ist sichtbar, lässt sich aber **nicht verschieben** | **behoben** in 7.2.0.30, von Gregor noch nicht bestätigt. Von ihm am 09.09.2026 an 1.0.29 gemeldet: *„rechts ist zwar ein balken sichtbar, aber nicht verschiebbar"*. **Ursache belegt:** `SECDockBar::CalcTrackingLimits` rechnete für **beide** senkrechten Andockleisten dieselben Grenzen — `min = Rahmen links + nMindest`, `max = Rahmen rechts − nFreiraum`. Für **links** stimmt das; für **rechts** ist es spiegelverkehrt, und zwar so, dass gar nichts mehr geht: der Balken sitzt dort an der **linken** Kante der Andockleiste, in deren Koordinaten `rectRahmen.left` stark negativ ist und `rectRahmen.right` bei etwa **188** liegt. Damit wird `max = 188 − 200 = −12`, also **kleiner als der Ort des Balkens**; die Notbremse `if (max < min + nMindest)` zieht `max` auf `min + 16` hoch, und zwischen zwei Grenzen 16 Pixel weit links vom Balken lässt sich nichts ziehen. Der Balken nimmt den Mausfang und bewegt sich nicht — genau das Bild. **Behebung:** die Seite, an der die Leiste **angewachsen** ist, bekommt `nMindest`; die Seite **zum MDI-Bereich** hin bekommt `nFreiraum`. **Nebenbei mitgefunden: UNTEN war ebenso falsch, nur unauffällig** — nach oben zu großzügig (der MDI-Bereich ließ sich auf 16 Pixel zusammenschieben), nach unten zu streng (die Leiste ließ sich nicht unter 200 Pixel Höhe verkleinern). Gregor hatte nur die eine Richtung geprüft (*„verschieben rauf / runter — bug gefixt"*), deshalb hat es niemand gemerkt: der Fall aus [[gegenprobe-umdrehen]] |
