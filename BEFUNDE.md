...
| Kennung | Worum es geht | Status |
|---|---|---|
| NP2-1 | Stufe 3 der Ersatzschicht hing in keinem Projekt | **ÃÂ¼berholt** |
| NP2-2 | `SysColorChange()` tut nicht, was daneben steht | **offen** (folgenlos, Kommentar irrefÃÂ¼hrend) |
| NP2-3 | `BitBltTransparent` meldet Erfolg, ohne gezeichnet zu haben | **offen** |
| NP3-1 | `ZeigeInhaltsfenster` lÃÂ¤uft, bevor Eudora das Fenster umhÃÂ¤ngt | **offen** (nur sichtbar, kein Datenfehler) |
| NP3-2 | `OnToolHitTest` als `int` statt `INT_PTR` Ã¢ÂÂ x64-Sperre | **offen** |
| NP3-3 | Kommentar zu `CreateFromBitmap` zÃÂ¤hlt die Aufrufstellen falsch | **offen** |
| NP3-4 | `lehren-spiegeln.pl` lÃÂ¶schte die gespiegelten Dateien wieder | **behoben** (stagt nicht mehr; Hook siehe X-2) |
...
| X-1 | neun LÃÂ¶cher in der Schranke, gegen die eigenen Werkzeuge gemessen | **behoben** Ã¢ÂÂ vollstÃÂ¤ndig, durch X-2, X-3 und X-4 |
| X-2 | die neun LÃÂ¶cher geschlossen, je mit Testfall; der Hook log | **behoben** im Skript Ã¢ÂÂ auf dieser Maschine war der *eingerichtete* Hook bis 05.09.2026 noch die alte Fassung (X-5) |
| X-3 | `suche-zeiger.pl` brauchbar gemacht: 347 Treffer auf 18, neun echte Kandidaten | **behoben** Ã¢ÂÂ die **neun Zeigerstellen** sind offen und brauchen einen Bau |
| X-4 | `zeilenenden-angleichen.pl`: 49 Dateien mehr, dreht keine absichtliche Arbeit mehr zurÃÂ¼ck | **behoben** |
| X-5 | Commit um 09:06 auf den um 09:03 zusammengefÃÂ¼hrten Zweig; die Regel stand nur als Prosa | **behoben** Ã¢ÂÂ Schranke im `pre-commit`, 15 TestfÃÂ¤lle, Auflagen 7Ã¢ÂÂ10 in `AUFGABEN.md` |
| X-6 | Bau-Lauf: geratene Plattform (`MSB4126`) und Erfolgsmeldung ohne Bau; `tools/bauen.ps1` | **behoben** Ã¢ÂÂ das Werkzeug steht, drei Gegenproben grÃÂ¼n |
| R-1 | die Fehlerklasse hinter E-11 ausgezÃÂ¤hlt | **offen** Ã¢ÂÂ gemessen am 07.09.2026 mit `tools/releasebuffer-pruefen.pl`: **137** Vorkommen, davon 116 richtig gepaart; **21** zu ÃÂ¤ndern (16 `falsch`, 4 `lockbuffer`, 1 `danach`). Zuerst `QCSharewareManager.cpp:1318` Ã¢ÂÂ die Stelle lÃÂ¤uft **bei jedem Start**. Liste in `AUFGABEN.md` unter A2, mit dem Werkzeug jederzeit neu zu erzeugen |
| Z-3 | erster Bau von Grund auf: `OEImport`/`NSImport` linken vor `QCUtils` Ã¢ÂÂ fehlende ProjektabhÃÂ¤ngigkeit in `Eudora.sln` | **behoben** (`57fe6a4`) Ã¢ÂÂ `ProjectReference` auf `QCUtils.vcxproj` steht in `NSImport`, `OEImport`, `OLImport` und `plstclnt`; am 06.09.2026 in allen vier Projektdateien nachgesehen |
| V-1 | zwei verschiedene ZIPs unter derselben Versionsnummer `v1.0.3` | **Regel jetzt durchgesetzt**, 14.09.2026. Die Regel stand seit dem 31.08.2026 in der Doku und hat nicht getragen, weil **nichts sie geprÃÂ¼ft hat** Ã¢ÂÂ `release-veroeffentlichen.ps1` legte ein Release an, ohne nachzusehen, ob die Marke schon existiert. Jetzt weist es ab (`gh release view <marke>`, RÃÂ¼ckgabe 0 = vorhanden), mit dem Hinweis, die Nummer hochzuzÃÂ¤hlen. **Gegengetestet in beide Richtungen:** `v1.0.3` Ã¢ÂÂ vorhanden, wird abgewiesen; `v1.0.53` Ã¢ÂÂ frei, lÃÂ¤uft durch. **Der Altbestand bleibt wie er ist:** unter `v1.0.3` hÃÂ¤ngt heute noch ein ZIP (9.209.393 B vom 31.08.2026), das ausgetauschte ist weg Ã¢ÂÂ nachtrÃÂ¤glich auseinanderhalten lassen sich die beiden nur ÃÂ¼ber die PrÃÂ¼fsummen in `Releases/PAKETE.md`. Gefunden hat das Gregor, nicht die Werkzeugkette |
...
| E-4 | Debug-Zusicherung beim Beenden: Index auÃÂerhalb `m_arrBars` | **behoben, ungeprÃÂ¼ft** in **7.2.0.3** (05.09., `1188e87`: die Ursache liegt beim **Start**, ein ungeprÃÂ¼fter Cast in `WazooBarMgr.cpp`) |
| E-5 | Ã¢ÂÂRelease startet auf Win11 gar nicht" | **ÃÂ¼berholt** durch E-6/E-8 |
| E-6 | Release lÃÂ¤uft, Assistent stÃÂ¼rzt bei *Weiter* ab | **ÃÂ¼berholt** durch E-8 (es war der Debug-Bau) |
| E-7 | die Bau-Kennung fehlt im Titel, solange kein Postfach offen ist | **behoben, ungeprÃÂ¼ft** in **7.2.0.3** (05.09., `bcc59bb`: ein `OnUpdateFrameTitle(TRUE)` in `mainfrm.cpp:1135`) |
| E-8 | Berichtigung zu E-6, und Kriterium 0 zurÃÂ¼ckgezogen | **Beleg** |
| E-9 | Absturz im Assistenten: die Kette bis zur Importsuche | **ÃÂ¼berholt** durch E-11; die HÃÂ¤rtung bleibt richtig |
| E-11 | `ReleaseBuffer` ohne `GetBuffer` in `eudora.cpp:3372` | **behoben, ungeprÃÂ¼ft** in **7.2.0.3** Ã¢ÂÂ und laut R-1 wahrscheinlich **unvollstÃÂ¤ndig** |
| E-12 | `Eudora.exe Mailverzeichnis` hielt das Verzeichnis fÃÂ¼r den Ini-**Dateinamen** | **behoben, ungeprÃÂ¼ft** in **7.2.0.4** (Kriterium 3) |
| E-13 | beim Mailabruf ist kein Fortschritt sichtbar Ã¢ÂÂ der Abruf dauert 0,02 s | **Ursache belegt**; Behebung liegt auf `wt/fortschritt-arbeit`, **nicht** in diesem Zweig. HieÃÂ versehentlich `E-12` |
| E-14 | Zusicherung beim Start: der X1-Suchindex wird neu angelegt (`SearchManager::Info::InitX1`) | **offen, nicht angefasst** Ã¢ÂÂ auf einem frischen Mailverzeichnis der normale erste Lauf; echter Befund erst, wenn die Meldung auch beim **zweiten** Start kommt |
| E-16 | Absturz beim Verfassen (Strg-N) und Ã¢ÂÂEncountered an improper argument", `afxcoll.inl:213` | **behoben** in **7.2.0.5** Ã¢ÂÂ sechs Indexstellen gehÃÂ¤rtet (`Befunde/VERFASSEN.md`). Die Ursache, die dieser Befund **nicht** fand, ist **E-34**: die Schranke aus E-16 greift, `m_btns[24]` wirft trotzdem, weil MFC 14 in den Sammlungen mit `ENSURE` statt `ASSERT` prÃÂ¼ft |
...
| E-86 | eine **Newsletter-Mail steht in Eudora auf weiÃÂem Grund, das Logo in einem Kasten mit blauem Rahmen** Ã¢ÂÂ im Webbrowser dagegen alles auf schwarzem Grund und das Logo ohne Rahmen | **offen; der erste Verdacht ist WIDERLEGT**, von Gregor am 14.09.2026 an 1.0.52 gemeldet. **Der Verdacht war:** `CTridentView::WriteTempFile` (`TridentView.cpp:1282-1481`) wickelt die Mail in ein eigenes HTML-Dokument (Zeichensatzzeile, `IDS_INI_READMESSAGE_STYLE_SHEET`, Kopfzeilen, `<div>`), die Mail bringt aber ihr **eigenes** vollstÃÂ¤ndiges Dokument mit `<html><head><style>Ã¢ÂÂ¦</style></head><body>` mit Ã¢ÂÂ MSHTML mÃÂ¼sse bei einem zweiten `<head>`/`<body>` deren Inhalt verwerfen, womit Hintergrundfarbe **und** `img{border:0}` wegfielen: beide Symptome aus einer Ursache. **Gemessen statt geschÃÂ¤tzt, und es stimmt nicht.** `tools/e86-fassung-bauen.pl` nimmt eine **echte** Nachricht aus `WaipuTV.mbx` und bildet den Weg des Programms nach (`AddBodyPart` nimmt den Bereich zwischen `<x-html>` und `</x-html>`, `msgutils.cpp:2365`; `AddToBody` entfernt bei `RunHtmlCode=0` Ã¢ÂÂ dem Vorgabewert, `EudoraRes.rc:8637` Ã¢ÂÂ nur `script`, `applet`, `object`, `embed`, `marquee`, **nicht** `html`/`head`/`body`, weil `bStripDocumentLevelTags` nur bei **mehreren** Nachrichten gesetzt wird, `TridentView.cpp:1409`); `tools/e86-messen.ps1` lÃÂ¤dt beide Fassungen in MSHTML und liest den **berechneten** Stil aus dem DOM. Ergebnis fÃÂ¼r **alle fÃÂ¼nf** Nachrichten des Postfachs, mit Bildern und mit Platzhaltern statt Bildern: `Stylesheets=3` (Eudoras **und** die beiden der Mail), `body-Hintergrund=#000000`, `img-in-a-Rahmen=0px`. Das Abbild zeigt schwarzen Grund und rahmenlose Bilder. **Der zweite `<head>`/`<body>` kostet den Hintergrund also nicht.** **Dazu passt, dass das Original den Fall lÃÂ¤ngst kennt:** `CTridentView::FixupSource` (`TridentView.cpp:1912`) existiert ausdrÃÂ¼cklich fÃÂ¼r das zweite `<body>`-Tag und kopiert dessen Attribute (`background`, `bgColor`, `text`, `link`, RÃÂ¤nder) auf das erste. **Die zweite Ansicht scheidet aus:** `UsingTrident()` hÃÂ¤ngt an `UseBidentAlways`, Vorgabe **1** (`EudoraRes.rc:8123`), Gregors `Eudora.ini` setzt den SchlÃÂ¼ssel nicht, und sein Protokoll vom 14.09.2026 enthÃÂ¤lt die Spurmarke `E-80 trident:` Ã¢ÂÂ angezeigt wird ÃÂ¼ber MSHTML, nicht ÃÂ¼ber Paige. **Was im Protokoll auffÃÂ¤llt:** jeder Aufbau meldet `konzentriert=1`; der Content Concentrator gibt einen **styled body** laut `ConConMessage.cpp:566-612` aber unverÃÂ¤ndert in GÃÂ¤nze aus. **NÃÂ¤chster Schritt, im Quellstand eingebaut (noch kein Paket gebaut, Stand 7.2.0.52):** zwei Spurmarken. `LoadMessage` legt die fertige Anzeigedatei bei eingeschaltetem Protokoll als **`E86-Anzeige.htm`** im Eudora-Verzeichnis ab Ã¢ÂÂ bisher lÃÂ¶schte der nÃÂ¤chste Aufbau sie (`unlink`), sie war nicht zu greifen; und `FixupSource` meldet in **einer** Zeile, ob es ÃÂ¼berhaupt lÃÂ¤uft, wie viele `BODY`-Elemente MSHTML angelegt hat und welchen Hintergrund es fÃÂ¼r den angezeigten `body` **berechnet** hat (`IHTMLCurrentStyle`). Damit entscheidet der nÃÂ¤chste Lauf den Fall, ohne dass noch einmal etwas nachgebaut werden muss | **Von Gregor ausdruecklich dazugenommen:** auch die **Skalierung der Bilder** stimmt nicht â im Original ist das Logo breiter und sitzt ohne Kasten. **Gemessen und geklaert ist**, dass es nur das **Lesen** betrifft: der HTML-Rahmen wird ausschliesslich in `TridentView.cpp` gebaut, der Verfassen-Weg (`PgCompMsgView`) fasst ihn nie an. Die Nachricht selbst bleibt unversehrt â wer sie weiterleitet, verschickt sie korrekt.
