...
| E-86 | **HTML-Nachrichten werden ohne ihr eigenes Aussehen dargestellt** — der Hintergrund eines Newsletters bleibt weiß statt schwarz, und verlinkte Bilder bekommen einen blauen Rahmen | **Verdacht am Quelltext, nicht bewiesen**, 14.09.2026, von Gregor mit Bildvergleich gemeldet (Original im Browser gegen Anzeige in 7.2.0.52). `CTridentView` baut die Anzeigedatei aus **Eudoras eigenem** Rahmen — `<meta charset>`, dann `IDS_INI_READMESSAGE_STYLE_SHEET` mit `<HTML><HEAD><STYLE>…</STYLE></HEAD><BODY>` (`TridentView.cpp:1325-1352`) — und hängt die Nachricht als `<div>…</div>` hinein. Die Nachricht ist aber selbst ein **vollständiges HTML-Dokument** mit eigenem `<head>` und `<body>`. MSHTML verwirft bei einem zweiten `<head>`/`<body>` deren Inhalt; damit fällt die Hintergrundfarbe am `<body>` der Mail weg **und** das `img{border:0}` aus ihrem `<style>`, weshalb ein verlinktes Bild den blauen Standardrahmen des IE zeigt. **Beide Symptome aus einer Ursache.** Zu messen mit einer Spurmarke, die die zusammengesetzte Datei festhält, oder durch Vergleich derselben Nachricht in beiden Ansichten (Paige/Trident) |
