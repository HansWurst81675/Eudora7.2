...
| E-84 | die **Größe eines losgerissenen Fensters überlebt den Neustart nicht** — es kommt beim nächsten Start wieder klein | **behoben** in 7.2.0.50, **noch nicht von Gregor bestätigt**. Von ihm am 11.09.2026 gemeldet, unmittelbar nach der Bestätigung von E-76: *„das undocked initiale fenster ist recht klein, die größe (nach der änderung) wird nach dem schließen vom filter und eudora nicht gespeichert. beim nächsten mal wieder klein."* **Ursache am Quelltext gelesen:** `SECControlBar::GetBarInfo` überträgt `m_szFloat` in die Info (`OTShim.cpp:2304`), `SetBarInfo` holt es zurück (`:2362`) — dazwischen ging es verloren, weil `SECControlBarInfo::SaveState` und `LoadState` **reine Weiterleitungen an MFC** waren, und MFC kennt `m_szFloat` nicht. Die Stingray-Felder wurden also in die Info geschrieben und beim nächsten Start nie gelesen. **Warum es so gebaut war:** der Kommentar in `SECControlBarInfo::Serialize` begründet, den SEC-Anteil wegzulassen — *„das Format müsste dann zum Original passen … eine eigene Erweiterung würde eine Datei erzeugen, die ein späterer echter Nachbau nicht mehr lesen kann"*. **Das Argument gilt für `Serialize` und nicht hier:** `Serialize` schreibt ein **Binärformat**, in dem ein zusätzliches Feld alles Nachfolgende verschiebt; `SaveState`/`LoadState` schreiben in die **INI**, wo ein zusätzlicher Schlüssel niemanden stört, der ihn nicht kennt, und bei seinem Fehlen der bisherige Wert gilt. **`Serialize` bleibt unberührt.** Behoben mit dem Schlüssel `BarFloatSize<n>`; eine Größe von 0 wird gar nicht erst geschrieben, weil `LoadState` sie sonst übernehmen müsste |
