...
| E-89 | **das Verfassenfenster ist unbrauchbar: Bilder überlagern den Text, man kann nicht lesen, was man schreibt** | **WIEDER OFFEN — die Behebung in 7.2.0.57 greift nicht.** Gregor am 14.09.2026 an 1.0.57, mit Bild: die Tonerkartusche liegt über den Zeilen *„Artikelnummer"* und *„Nachbestellung mit Order-No"*, darunter ein leerer grauer Kasten. Sein Urteil: *„findest du, es ist leserlich? ich nicht!"* **Die eingebaute Spurmarke widerlegt die angenommene Ursache** — aus seinem Lauf: `E-89 Bilder im Editor: gesamt=25 unveraendert=22 aus-CSS=0 Vorgabe=3 gedeckelt=0 geaendert=1 Bytes vorher=62057 nachher=62105`. **`aus-CSS=0`**: kein einziges Bild dieser Nachricht hatte seine Größe im CSS — genau der Fall, für den die Behebung gebaut wurde. **`gedeckelt=0`**: keines war zu breit. 22 von 25 trugen bereits brauchbare Attribute, geändert wurden **48 Bytes von 62.057**. Die Bilder überlappen also, obwohl ihre Größe stimmt. **Damit ist gemessen: die fehlende Bildgröße war nicht die Ursache.** Wenn ein Bild richtig bemaßt ist und trotzdem über dem Text liegt, rechnet Paige die **Position** falsch, nicht die Ausdehnung. **Zu prüfen, nicht zu vermuten:** (1) `PgEmbeddedImage.cpp:458-507` sucht die Einfügestelle über `embed->style == embed_ptr->style` und bleibt beim **ersten** Bild gleichen Stils stehen — haben mehrere Bilder denselben Stil, landen sie womöglich alle an derselben Stelle, und das träfe auch Bilder **mit** Größe; (2) setzt Paiges HTML-Leser `<td>` in eine Spaltenstruktur um oder hängt er den Inhalt hintereinander; (3) was macht `PGHTMIMP` mit `align` an einem `<img>`. **Erst eine Spurmarke mit der berechneten Position je Bild, dann ändern** — eine zweite Fassung, die wieder nicht wirkt, kostet mehr als eine Stunde Messen. **Eigener Rückschritt, der zurückzunehmen ist:** die Vorgabe 200x90 für Bilder ohne bekannte Größe (`Vorgabe=3`) erzeugt den leeren grauen Kasten auf Gregors Bild. Vorher stand dort nichts. **FUNDSTELLE MIT BELEG, 14.09.2026 nach Gregors Ablehnung** (vollständig in [Befunde/PRUEFER-ZWISCHENSTAND.md](Befunde/PRUEFER-ZWISCHENSTAND.md), Zweig `wt/pruefer`, Commit `90679db`): **`PGHTMIMP.CPP:2109-2116`** — `current_style.ascent = image_record.source_height;` wird in der **nächsten Zeile** von `original_descent` überschrieben, einem Wert aus dem **Textstil**, der aufgenommen wurde, bevor das Bild bekannt war. Übrig bleibt `ascent = max(Text-Ascent, Text-Descent)`: **die Bildhöhe geht in die Zeilenhöhe überhaupt nicht ein.** Eine Schicht tiefer rechnet `pgInitEmbedStyleInfo` → `EMBED_VMEASURE` (`PGEMBED.C:1666-1681`) genau diesen Wert korrekt aus der Bildhöhe (`if (style->ascent < embed_ptr->height) style->ascent = embed_ptr->height;`) — der HTML-Leser wirft ihn eine Zeile später weg. Ein 150 Punkt hohes Bild sitzt dann in einer textkleinen Zeile und deckt die Zeilen darüber zu. **Das ist Gregors Bild.** Kein Portierungsfehler: identisch in allen vier Fassungen im Repo. **Zwei Erklärungen sind gemessen ausgeschlossen:** der Nachtrag über `embed->style` steht unter `if ((embed->height == 0) || (embed->width == 0))` und startet für die 22 bemaßten Bilder gar nicht; der Rahmenweg (`PGHTMIMP.CPP:2045`) greift nur bei `align`, und von 3060 `<img>` in den Postfächern tragen **16** ein `align`, alle mit Wert `middle` — der setzt `frame.data` gerade nicht. **Noch nicht gemessen und vor jeder Änderung zu messen:** ob `current_style` an dieser Stelle wirklich der Stil ist, der für das Embed in den Text geht, oder ob `ProcessEmbed()` einen anderen nimmt |
