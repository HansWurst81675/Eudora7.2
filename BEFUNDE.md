...
| E-85 | **Umlaute in per IMAP abgerufenen Nachrichten kommen falsch an** | **Ursache am Quelltext gefunden**, 13.09.2026, von Gregor am selben Tag gemeldet. **Drei Mängel im IMAP-Weg, alle in `EuImap/src/ImapDownload.cpp`.** **(0) Der schwerwiegendste: `charset=utf-8` wird gar nicht erkannt.** Zeile 4645 sucht mit `FindRStringIndexI(IDS_MIME_US_ASCII, IDS_MIME_ISO_LATIN9, …)` — der Bereich endet bei **3613**, `IDS_MIME_UTF_8` ist **3614** und liegt damit außerhalb. Der Aufruf liefert `-1`, `if (iCharsetIdx > 1)` ist falsch, und **es wird überhaupt nicht übersetzt**; die UTF-8-Bytes gehen roh in die Mailboxdatei und werden später als CP1252 angezeigt. Der POP3-Weg macht es richtig: `FindMIMECharset` (`Eudora/mime.cpp:382-402`) sucht bis `IDS_MIME_UTF_8` **und** verschiebt das Ergebnis um eins (`++iCharSet`), damit Index 0 für `windows-*` frei bleibt. Genau diese Verschiebung fehlt im IMAP-Weg ebenfalls — die beiden Wege rechnen auf **verschiedenen Skalen**, was der bestehende Test *„POP: der Index wird um eins verschoben — anders als im IMAP-Pfad"* schon festhielt. **(1) behoben in 7.2.0.51:** Zeile 4662 verwarf den Rückgabewert von `ISOTranslate`; `outLen` behielt die Länge *vor* der Übersetzung und genau die wurde geschrieben. **(2) behoben in 7.2.0.51:** die Behebung von **Z-2b** (angefangene UTF-8-Zeichen an der Stückgrenze zurückhalten) stand nur in `TextReader.cpp` und damit nur im POP3-Weg; IMAP liest bei `text/html` in Blöcken von 8192 Bytes. Neu dafür: `ISOTranslateChunk` in `utils.cpp`, mit vier Tests über alle Stückgrößen. **Belegt am Quelltext, noch nicht am laufenden Programm** |
