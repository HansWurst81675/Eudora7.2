...
| E-89 | **im Verfassenfenster liegen die Bilder über dem Text** — *„1 und da ist alles durcheinander, man kann ja nichts lesen."* Gregor am 14.09.2026 an 1.0.56, mit einem Bildschirmfoto: das Doctolib-Logo liegt über der Überschrift, die App-Store-Kacheln liegen über einem Absatz. Das Fenster ist zum Arbeiten unbrauchbar | **behoben in 7.2.0.57, Bau 0 Fehler, Tests 148/148 (17 davon neu zu E-89, sieben Gegenproben) — von Gregor noch nicht am laufenden Programm bestätigt.** **Der Maßstab ist Thunderbird**, aber nur seine Schwelle, nicht sein Aussehen: dessen Verfassenfenster zeichnet rote Hilfslinien um jede Tabellenzelle und ist auch nicht schön — aber **nichts überlappt**, und jeder Satz ist lesbar. Thunderbird rechnet mit Gecko, Paige mit einem eigenen Format; das Bild links ist nicht erreichbar, die Lesbarkeit schon. **Warum das überhaupt geändert werden darf:** seit **E-88** geht beim Senden das aufgehobene Original hinaus, nicht die Editorfassung — das Verfassenfenster ist nur noch Arbeitsfläche. **Zwei Ursachen, beide gemessen:** (1) Paiges HTML-Leser holt die Bildgröße **ausschließlich** aus den Attributen `width` und `height` (`PGHTMIMP.CPP:2019-2022`); steht sie nur im CSS, bleiben `source_width`/`source_height` auf `0`, und `PgEmbeddedImage.cpp:424-427` trägt beim **Laden** die Originalgröße nach — da war der Absatz längst umbrochen. Der Nachtrag sucht seine Stelle über `embed->style == embed_ptr->style` (`PgEmbeddedImage.cpp:458-507`) und bleibt beim **ersten** Bild gleichen Stils stehen; bei mehreren Bildern trifft er die falsche. Das ist das Übereinanderliegen. (2) Auch eine **lesbare** Größe kann zu groß sein: Gregors zweites Thunderbird-Bild (freenet/audibene) zeigt dasselbe Kopflogo im schmalen Verfassenfenster schmal und in der breiteren Leseansicht breiter — proportional auf die Spaltenbreite gerechnet. **Die Behebung:** `E89BilderMessbarMachen` (`msgutils.cpp`) gibt jedem `<img>` in der Fassung, die in den Editor geht, eine auswertbare Größe — aus `style="width:16px"` wird `width="16"`, aus `style="width:100%"` wird `width="100%"` (`decimal_value_percent` kann Prozent, `numeric_value` für die Höhe nicht) — und deckelt sie bei **600 Bildpunkten** in beiden Richtungen, das andere Maß proportional mit. Wo gar nichts steht, greift 200x90; es gibt dort nichts zu rechnen, und nachgeladen wird beim Verfassen nichts. **Gemessen** an Gregors gesicherten Postfächern: 2884 `<img>`, davon 2601 mit Attribut, 122 nur mit CSS, 161 ganz ohne Größe. **Warum nicht der Platzhalter** (`[Bild]` statt des Bildes): das würde **E-88 zerstören** — `E88NurText` wirft Markierungen weg, ein zusätzliches `width=` ist für den Vergleich unsichtbar, ein eingefügtes `[Bild]` zerreißt den Klartext mitten im Zitat, und das Original ginge nie wieder hinaus. Ein Test fährt genau diese Gegenprobe. **Das Original bleibt unberührt:** `summary.cpp` trennt die beiden Fassungen an **einer** Stelle (`EditorBody` gegen `Body`), beide hängen an derselben Bedingung `bE88Merken`. Ist `ForwardOriginalHTML=0`, geht die Editorfassung selbst hinaus — dann unterbleibt auch die Umschrift. **Ein eigener Schalter ist nicht nötig und wurde nicht angelegt.** **Gemessen statt vermutet, dass kein echtes Fenstermaß zu haben ist:** der Aufruf geht mit `kDontDisplayOrQueue` hinein (`summary.h:142`), beide Zweige, die ein Fenster erzeugen könnten, hängen daran (`summary.cpp:1818-1837`) — zur Umschreibzeit existiert das Verfassenfenster nicht, und ein gespeichertes Maß gibt es auch nicht. **Eine Spurmarke** (`DEBUG_MASK_MISC`) nennt in einer Zeile Bildzahl, unverändert, aus CSS, Vorgabe, gedeckelt und beide Bytelängen. **Grenzen, die bleiben:** Kästen, Rahmen und Hintergründe erscheinen im Verfassenfenster weiterhin nicht (Paige kennt kein CSS); die Tabellenaufteilung wird nicht nachgebaut; und wo keine Größe bekannt ist, ist 200x90 geraten und kann verzerren. Dokumentiert in [EINSTELLUNGEN.md](EINSTELLUNGEN.md), Abschnitt 4.8 |
