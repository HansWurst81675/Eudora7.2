...
| E-33 | *File → Exit*, das **Kreuz** und **Alt-F4** beenden Eudora nicht, sondern bringen die Meldung **„Encountered an improper argument"** (Kriterium 7) | **offen, Weg vermessen, Ursache nicht belegt.** Gregor am 07.09.2026 an Paket 1.0.21, mit Bildschirmfoto: *„exit: weder alt+F4, noch x rechts oben funktionieren. da kommt wieder die meldung"*. Damit ist belegt: das Beenden **beginnt** (alle drei Wege gehen durch dasselbe `CMainFrame::OnClose`, `CFileBrowseView::OnAppExit` ist ausgeschlossen), der Abbruch ist eine **geworfene `CInvalidArgException`** und keine stille FALSE-Rückgabe, und `CWinApp::ProcessWndProcException` (`appcore.cpp:1009-1039`) zeigt die Meldung und liefert 0 — `WM_CLOSE` gilt als beantwortet, das Fenster bleibt. Weil es danach noch da ist, muss der Wurf **vor** `pApp->HideApplication()` (`winfrm.cpp:885`) fallen. **Verdacht:** `QCCustomToolBar::SaveCustomInfo` (`Eudora71/Eudora/QCCustomToolBar.cpp:421`) — dieselbe Form wie E-34 und im Betrieb nur beim Beenden erreicht. 28 Spurmarken liegen, der Beleg fehlt (`Befunde/BEENDEN.md`) |
...
| E-37 | **ein Konto lässt sich nicht löschen** — Rechtsklick in der Personalities-Liste, *Delete*, Rückfrage mit *Ja*, und die Liste bleibt unverändert stehen | **behoben** (07.09.2026). Gregors Messung hat die erste Annahme widerlegt: *„ja, sie verschwinden nach neustart"* — gelöscht wird also korrekt, nur die **Anzeige** wird nicht nachgezogen. `FindItem` liefert −1, `DeleteItem(−1)` tut nichts, und abgesichert war das nur mit `ASSERT(nIndex != -1)` — im Release-Bau ein Nichts. Behoben in `CPersonalityView::OnCmdDeletePersonality` (`Eudora71/Eudora/PersonalityView.cpp`) unabhängig davon, **warum** `FindItem` scheitert: der Fehlschlag geht mit Name und Listenlänge ins Protokoll, und die Liste wird einmal am Ende über `PopulateView()` neu aufgebaut. Die drei stummen `ASSERT(0)`-Zweige melden jetzt ebenfalls ins Protokoll. **Warum `FindItem` −1 gibt, ist offen** — die Protokollzeile beim nächsten Lauf sagt es |
| E-38 | **die im Kontoassistenten eingegebenen Daten fehlen hinterher** — Name, Mailadresse und Server sind unter *Konto → Eigenschaften* leer | **offen** — von Gregor am 07.09.2026 gemeldet: *„obwohl daten (name, mailadresse, server) im wizard eingetragen werden, fehlen diese beim konto->eigenschaften!"* Zwei Kandidaten, beide belegt gelesen, keiner gemessen: **(a)** `OnSetActive` der Serverseiten setzt erst `m_str… = Params->…` und ruft dann `UpdateData(TRUE)` — also in die **falsche** Richtung, aus dem Steuerelement in die Variable (`WizardInServerPage.cpp:116-119`, `WizardOutServerPage.cpp:73-83`); über beiden steht die richtige Fassung `//UpdateData(false)` auskommentiert. Ist das Feld beim Aktivieren noch leer, wird der Wert überschrieben. **Beide Zeilen stammen aus dem Original von 2006** (`git log -L`, Commit `567a5d8`), sind also keine Regression der Portierung — aber unter MFC 14 kann die Aufbaureihenfolge der Seite anders sein. **(b)** `CPersonality::SavePersonaInfo` (`persona.cpp:924-1070`) bricht beim **ersten** fehlgeschlagenen `WriteStrHelper` mit FALSE ab und lässt alles Folgende ungeschrieben; der Aufrufer verpackt das in `VERIFY(g_Personalities.Add(...))` (`AccountWizard/Src/WizardPropSheet.cpp:193`), und `VERIFY` prüft im Release-Bau **nicht**. Die Reihenfolge passt zum Symptom: POPAccount, RealName, ReturnAddress, DefaultDomain, SMTPServer, POPServer — nach einem Fehlschlag am Anfang fehlt genau das, was Gregor nennt. **Erste Messung ohne Bau:** in der `Eudora.ini` des Mailverzeichnisses den Abschnitt `[Persona-<Name>]` ansehen. Stehen `RealName`, `ReturnAddress` und `SMTPServer` dort leer oder fehlen sie, ist es die Schreib- oder Übertragungsseite; stehen sie richtig da, liegt der Fehler beim **Lesen** (`GetParams`, `persona.cpp:279ff`, über `VERIFY` in `ModifyAcctSheet.cpp:47` ebenfalls stumm) |
