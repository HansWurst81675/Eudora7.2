...
| E-37 | **ein Konto ließ sich scheinbar nicht löschen** — Rechtsklick, *Delete*, Rückfrage mit *Ja*, und die Liste blieb unverändert stehen | **nur die ANZEIGE behoben — die Ursache ist offen.** Gregors Messung: *„ja, sie verschwinden nach neustart"* — gelöscht wurde immer korrekt. `FindItem` liefert −1, `DeleteItem(−1)` tut nichts, abgesichert nur mit `ASSERT`. Behoben in `CPersonalityView::OnCmdDeletePersonality` (`Eudora71/Eudora/PersonalityView.cpp`) unabhängig davon, warum `FindItem` scheitert: Fehlschlag ins Protokoll, Liste einmal am Ende über `PopulateView()` neu aufgebaut, die drei stummen `ASSERT(0)`-Zweige melden jetzt. **Was NICHT behoben ist:** (a) **warum** `FindItem` den Eintrag nicht findet, obwohl Spalte 0 den rohen Namen trägt (`:222-232`) und dieselbe Zeichenkette bei `Remove` erfolgreich war — das sagt erst die neue Protokollzeile; (b) das Löschen der **aktuell benutzten** Persönlichkeit, siehe **E-39**. Von PRUEFER am 07.09.2026 nachgeprüft: *„teilweise bestätigt — die Behebung wirkt und richtet keinen neuen Schaden an"* (`Befunde/PRUEFER-4.md`). **Wer daraus liest „Löschen ist gefixt", liest zu viel** |
