...
| E-37 | **ein Konto lässt sich nicht löschen** — Rechtsklick in der Personalities-Liste, *Delete*, und es passiert nichts | **offen** — von Gregor am 07.09.2026 an Paket 1.0.21 gemeldet: *„löschen der konten geht übrigens auch nicht: auf toFix liste!"* `CPersonalityView::OnDeletePersonality` (`Eudora71/Eudora/PersonalityView.cpp:925-976`) behandelt **jeden** Fehlschlag mit `ASSERT(0)` — vier Stellen, und `ASSERT` ist im Release-Bau nichts. Drei lautlose Ausgänge: `GetSelectedPersonalities` gibt 0 (`:273-298`), `CPersonality::Remove` findet den Eintrag nicht in `[Personality]` der `Eudora.ini` (`persona.cpp:531-540`), oder `FindItem` gibt −1 und `DeleteItem(−1)` läuft ins Leere (`:962-965`). Ein vierter Ausgang meldet sich wenigstens: laufende Aufgaben (`persona.cpp:526-530`, Text `IDS_CANT_REMOVE_PERSONA`). Welcher es ist, trennt man **ohne Bau**: Konto anklicken, `Entf` drücken und sehen, welcher Dialog kommt — keiner, „active task(s)", oder die Rückfrage und danach nichts |
