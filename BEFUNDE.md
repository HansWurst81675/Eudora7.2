...
| E-46 | Verdacht: `CMainFrame::OnClose` arbeitet nach `QCWorkbook::OnClose` noch **rund 230 Zeilen** weiter — auf einem bereits **freigegebenen** Objekt | **widerlegt** (7.2.0.23) — die Spurmarke `E-46 CMainFrame::~CMainFrame betreten` erscheint im Protokoll **nach** `nach QCWorkbook::OnClose`, nicht dazwischen. Das Objekt ist an der Stelle in `SaveCustomInfo` also **nicht** freigegeben, und **E-43** hat damit seine Ursache **nicht** gefunden. Die Marke bleibt drin, weil sie die Reihenfolge dauerhaft belegt. Die Beweisführung, die zum Verdacht führte: `CFrameWnd::PostNcDestroy` ist wörtlich `delete this` (MFC 14, `winfrm.cpp:269-275`), `CMainFrame` überschreibt es nicht (im Bestand kein einziges `PostNcDestroy`), und `CFrameWnd::OnClose` endet mit `DestroyWindow()` (`winfrm.cpp:941`), das `WM_NCDESTROY` **synchron** zustellt. Der Aufruf steht aber **mitten** im Rumpf (`mainfrm.cpp`, Schritt 6 von 12), danach folgen `TocCleanup`, `TrimJunk`, `SaveBarState("ToolBar")` und acht weitere Schritte. Wäre der Verdacht richtig, hätte **E-43** damit seine Ursache und die krummen Fensterlagen ihre Erklärung. **Entschieden wurde es an genau dieser Protokollzeile:** stünde `E-46 CMainFrame::~CMainFrame betreten` **zwischen** `vor QCWorkbook::OnClose` und `nach QCWorkbook::OnClose`, wäre der Verdacht belegt — sie steht **danach**. Die Marke greift nichts an und bleibt drin, weil sie die Reihenfolge dauerhaft belegt |
...
| E-37 | **ein Konto lässt sich nicht löschen** — Rückfrage mit *Ja*, danach bleibt der Eintrag in der Liste stehen | **NICHT behoben.** Zwei Anläufe, der erste war eine **Regression**: mein `PopulateView()`-Aufruf hat geworfen, die Ausnahme lief aus dem Befehlsbehandler heraus, und der Anwender sah **„Encountered an improper argument"**. Gregor am 08.09.2026: *„die meldung kommt, wenn ich eine persona gelöscht habe"* und *„sie verschwindet links nicht, bis ich eudora geschlossen habe"*. Gelöscht **wird** korrekt (er hatte es am 07.09. gemessen: *„ja, sie verschwinden nach neustart"*) — es ist ein Anzeigefehler. Der zweite Anlauf sucht den Eintrag über `theCtrl.GetItemText(i, 0) == strName` statt über `FindItem` und ruft `PopulateView()` **nicht** mehr; gebaut, **von Gregor noch nicht bestätigt**. Offen bleibt außerdem, **warum** `FindItem` −1 liefert und **warum** `PopulateView` wirft |
| E-38 | **die im Kontoassistenten eingegebenen Daten fehlen hinterher** — Name, Mailadresse und Server sind unter *Konto → Eigenschaften* leer | **offen, hängt an E-33.** Von Gregor am 07.09.2026 gemeldet und dann nachgemessen: *„E-38: stand drin, ja"* — in der `Eudora.ini` **stehen** die Werte. Damit scheitert das **Lesen** (`GetParams`, `persona.cpp:279ff`, über `VERIFY` in `ModifyAcctSheet.cpp:47` stumm), oder die Werte gehen verloren, weil Eudora nur per `pkill` zu beenden ist. Sein Wort: *„vielleicht fehlen die daten, wenn ich eudora per task manager abschließen muß."* **Erst nach E-33 zu messen** — vorher ist jede Aussage dazu wertlos. Der zweite Kandidat bleibt notiert: `OnSetActive` der Serverseiten ruft `UpdateData(TRUE)` in die falsche Richtung (`WizardInServerPage.cpp:116-119`, `WizardOutServerPage.cpp:73-83`), beide Zeilen aus dem Original von 2006 (`567a5d8`) |
...
> **Nachtrag 08.09.2026: der Verdacht ist widerlegt.** Die Spurmarke
> `E-46 CMainFrame::~CMainFrame betreten` erscheint im Protokoll **nach**
> `nach QCWorkbook::OnClose`, nicht dazwischen — das `CMainFrame`-Objekt ist an
> der Stelle in `SaveCustomInfo` also **nicht** freigegeben. **E-43** hat damit
> seine Ursache **nicht** gefunden; es bleibt bei „der übersetzte Code liest an
> zwei verschiedenen Adressen". Die Beweisführung unten bleibt als vermessener
> Weg stehen, und die Marke bleibt im Quelltext, weil sie die Reihenfolge
> dauerhaft belegt.
