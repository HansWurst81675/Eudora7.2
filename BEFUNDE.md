...
| E-85 | **Umlaute in per IMAP abgerufenen Nachrichten kommen falsch an** | **Ursache am Quelltext gefunden**, 13.09.2026, von Gregor am selben Tag gemeldet. **Zwei Mängel im IMAP-Schreibweg, beide in `EuImap/src/ImapDownload.cpp`:** (1) `ISOTranslate` wird in Zeile 4662 gerufen, sein **Rückgabewert aber verworfen** — `outLen` behält die Länge *vor* der Übersetzung, und genau die wird in Zeile 4709 geschrieben (`m_mbxFile.Put(pBuf, outLen)`). Da UTF-8 beim Übersetzen nach CP1252 **kürzer** wird, landen die überzähligen Altbytes mit in der Mailboxdatei. Der POP3-Weg macht es richtig: `size = ISOTranslate(buf, size, iCharsetIdx)` (`Eudora/TextReader.cpp:300`). (2) Die Behebung von **Z-2b** — angefangene UTF-8-Zeichen an der Stückgrenze zurückhalten — steht **nur** in `TextReader.cpp` und wirkt damit nur für POP3. IMAP liest ebenfalls stückweise (`CChunkReader::GetNextChunk`, `ImapDownload.cpp:4513/4521`), hält aber nichts zurück. **Belegt am Quelltext, noch nicht am laufenden Programm** |
