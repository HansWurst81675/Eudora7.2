...
| E-86 | 1.0.54 | *steht aus* | Zwei Marken, eingebaut am 14.09.2026, nachdem der erste Verdacht **widerlegt** war: MSHTML verwirft die Stylesheets der Nachricht **nicht** (gemessen an fünf echten Nachrichten, `Stylesheets=3`, Hintergrund `#000000`, Bildrahmen `0px`). **(1)** `LoadMessage` legt die fertige Anzeigedatei als `E86-Anzeige.htm` im Eudora-Verzeichnis ab — bisher löschte der nächste Aufbau sie, sie war nicht zu greifen. **(2)** `FixupSource` meldet in **einer** Zeile, ob es läuft, wie viele `BODY`-Elemente MSHTML angelegt hat und welchen Hintergrund es berechnet (`IHTMLCurrentStyle`). Auszuwerten, sobald Gregor die Newsletter-Mail in 1.0.54 öffnet: die Datei und die Zeile `E-86 fixup:` entscheiden zusammen, wo die Formatierung verlorengeht |
