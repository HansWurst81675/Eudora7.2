...
| | |
|---|---|
| **Quellstand** | 7.2.0.54 (`Eudora71/Version.h`) |
| **Paketnummer** | 1.0.54 (`VERSION`) |
| **Zuletzt gebaut und gepackt** | Paket **1.0.51** (Quellstand 7.2.0.51), lokal unter `C:\Users\Gregor\Eudora72-1.0.51-release`, von Gregor am 13.09.2026 getestet. **Auf GitHub veröffentlicht ist `v1.0.50`** — 1.0.51 ist **nicht freigegeben**, weil sein Test den Fehler E-85 weiterhin zeigt |
...
| **Als Nächstes, mit Gregor abgesprochen** | **1. E-85 — Umlaute in per IMAP abgerufenen Nachrichten: behoben in 7.2.0.51, von Gregor noch nicht geprüft.** Von ihm am 13.09.2026 gemeldet und der Vorschlag angenommen (*„e-85 vorschlag akzeptiert"*), Zweig **`fix-imap_utf8`** angelegt und gefüllt; Befundeintrag steht unter **E-85** in `BEFUNDE.md`. **Sein Hinweis hat sich bestätigt** (*„das hatten wir ja bereits gefixt, soweit ich mich erinnern kann?"*): die Behebung von **Z-2b** stand nur in `TextReader.cpp` und deckte damit allein den POP3-Weg. Drei Mängel, alle behoben — `ImapDownload.cpp` ruft jetzt `FindMIMECharset` (`:4671`) statt einer eigenen Suche, die `utf-8` gar nicht fand; der Rückgabewert von `ISOTranslate` wird ausgewertet; `ISOTranslateChunk` (`utils.cpp:1269`) trägt angefangene UTF-8-Zeichen über die Stückgrenze. **Am Quelltext belegt, noch nicht am laufenden Programm.** **Eine Regression kam durch die Behebung selbst hinzu und ist mitbehoben:** `ISOTranslate` schreibt die abschließende Null an `szBuf[lSize]`, ein Byte hinter den Bereich; der POP3-Weg hält dafür Platz frei, der IMAP-Weg nicht, sodass bei `text/plain; charset=utf-8` das **erste Byte der nächsten Zeile** getroffen wurde. Von **PRUEFER** am 13.09.2026 gefunden (`Befunde/PRUEFER-10.md`) und in `ISOTranslateChunk` geschlossen, indem das Byte hinter dem Stück gerettet und zurückgeschrieben wird (`utils.cpp:1308-1310` und `:1393-1395`). **Nächster Schritt: Paket bauen und Gregors Urteil einholen.** **2. QCSSL in `Releases/1.0/` hinkt.** Dort liegt `QCSSL 1.0.1` vom 30.08.2026, also **ohne** den E-82-Zertifikatsfix vom 11.09.2026; `tools/release-pruefen.pl` weist deshalb ab. Betroffen ist **nur** diese Nebenablage für 7.1-Anwender („zwei Dateien austauschen") — das Paket 1.0.50 selbst enthält die richtige DLL. Zu tun: Kennung in `Eudora71/QCSSL/src/qcssl.rc:14-15` auf 1.0.2, QCSSL neu bauen, `Releases/1.0/QCSSL.dll` und `.sha256` ersetzen, Zeile in `Releases/1.0/AUSLIEFERUNGEN.md`. **Reihenfolge noch offen** — Gregor gefragt, vor dem Herunterfahren nicht mehr beantwortet |
