...
| **Als Nächstes, mit Gregor abgesprochen** | **1. E-85 — Umlaute in per IMAP abgerufenen Nachrichten kommen falsch an.** Von Gregor am 13.09.2026 gemeldet und der Vorschlag angenommen (*„e-85 vorschlag akzeptiert"*), Zweigname **`fix-imap_utf8`** vereinbart. Noch **kein** Befundeintrag geschrieben, noch **kein** Zweig angelegt — beides ist der erste Schritt. Sein Hinweis: *„das hatten wir ja bereits gefixt, soweit ich mich erinnern kann?"* — also zuerst nachsehen, welche Stelle damals geändert wurde und warum sie den IMAP-Weg nicht deckt. **2. QCSSL in `Releases/1.0/` hinkt.** Dort liegt `QCSSL 1.0.1` vom 30.08.2026, also **ohne** den E-82-Zertifikatsfix vom 11.09.2026; `tools/release-pruefen.pl` weist deshalb ab. Betroffen ist **nur** diese Nebenablage für 7.1-Anwender („zwei Dateien austauschen") — das Paket 1.0.50 selbst enthält die richtige DLL. Zu tun: Kennung in `Eudora71/QCSSL/src/qcssl.rc:14-15` auf 1.0.2, QCSSL neu bauen, `Releases/1.0/QCSSL.dll` und `.sha256` ersetzen, Zeile in `Releases/1.0/AUSLIEFERUNGEN.md`. **Reihenfolge noch offen** — Gregor gefragt, vor dem Herunterfahren nicht mehr beantwortet |
