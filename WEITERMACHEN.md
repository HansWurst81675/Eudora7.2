...
| **Paketnummer** | 1.0.74 (`VERSION`) — Messfassung, behebt **nichts**. Die E-110-Spurmarke sitzt jetzt in `PgLoadUrlImage` selbst und nennt zusätzlich `datei=` und `intern=`: damit ist zu trennen, ob eine Bilddatei gar nicht ankommt oder ankommt und nicht umgewandelt werden kann |
