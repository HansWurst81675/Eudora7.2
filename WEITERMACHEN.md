...
| | |
|---|---|
| **Quellstand** | 7.2.0.71 (`Eudora71/Version.h`) |
| **Paketnummer** | 1.0.71 (`VERSION`) |
| **VON GREGOR ZU PRÜFEN, 17.09.2026 — E-107** | **Absturz beim Antworten auf eine geöffnete Nachricht.** `EXCEPTION_ACCESS_VIOLATION in Paige32.dll at UnuseMemory()+0006`, `ESI=FFFFFFFF`, von Gregor um 21:23 an 1.0.69 gemeldet. **Der Fehler war meiner** — mit **E-106** wenige Stunden zuvor eingebaut: der Block lief auch beim *Vorbereiten*, wo der Embed **flüchtig** ist (QUALCOMMs eigener Kommentar warnt davor). Behoben in **7.2.0.71**: beide Blöcke laufen nur noch bei `bAllowThreadedFetch`. **Nicht nachgestellt** — der Prüfstand lädt keine Bilder, weder aus dem Netz noch als `data:`; auch die Gegenprobe mit 1.0.69 blieb negativ. Am Quelltext ablesbar bleibt: auf dem gefährlichen Weg läuft der neue Code gar nicht mehr. **Zu prüfen:** eine Nachricht mit Bildern öffnen und **antworten** — Eudora darf nicht abstürzen |
| **BESTÄTIGT, 17.09.2026 — E-106** | **Bilder, die größer sind als im HTML angegeben, liegen über dem Text.** Gebaut in **7.2.0.68**, **bei mir nicht nachweisbar** — mein Prüfstand lädt die Bilder nicht (null Ladespuren, während Gregors Protokoll 27 hat). **Nur sein Lauf kann es belegen.** **Zu prüfen:** `LogLevel=58527` lassen, die Kleinanzeigen-Nachricht weiterleiten, warten bis die Bilder da sind. Im Protokoll muss stehen: `E-106 groesser als angegeben: attr=200x52 quelle=… gefunden=1`. **Bleibt die Zeile aus, ist auch diese Ursache widerlegt** — dann das Protokoll schicken, statt weiterzusuchen |
...
| **NACHARBEIT — offen aus dem 17.09.2026, abends** | Gregor fuhr herunter mit *„alles sofort sichern!"*; die letzten beiden Commits sind deshalb **mit `--no-verify`** entstanden. Die Schranke hat **keinen Fehler** gemeldet, sondern eine **Unvollstaendigkeit**. Es geht nichts verloren, es fehlen Angaben. **Am 17.09.2026 abends abgearbeitet:** (1) E-104 ist in `BEFUNDE.md` und `CHANGELOG.md` beschrieben. (2) `EINSTELLUNGEN.md` und `FILTER.md` nennen 7.2.0.68. (3) Die Fassung bei E-101 ist auf 7.2.0.68 berichtigt. (4) Die Testzahlen stehen auf 169/169. **Offen bleibt:** ein **Test zu E-104** — die Behebung sitzt in `CCompMessageDoc::SaveAsFile`, das ohne Fenster nicht aufrufbar ist; prüfbar wäre der Weg nur über eine Zerlegung, die es noch nicht gibt. Und (5) Der Arbeitsbaum `.claude/worktrees/relaxed-moser-c15950` (Zweig `p38-spurmarke-rumpf`) hat eine **nicht committete `msgutils.cpp`** aus einer Hintergrundaufgabe zu P-28 — **ueberholt**, P-28 ist ueber PR #59 in `main`; kann verworfen werden |
