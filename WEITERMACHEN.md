...
| | |
|---|---|
| **Quellstand** | 7.2.0.67 (`Eudora71/Version.h`) |
| **Paketnummer** | 1.0.67 (`VERSION`) |
| **NACHARBEIT — offen aus dem 17.09.2026, abends** | Gregor fuhr herunter mit *„alles sofort sichern!"*; die letzten beiden Commits sind deshalb **mit `--no-verify`** entstanden. Die Schranke hat **keinen Fehler** gemeldet, sondern eine **Unvollstaendigkeit**. Es geht nichts verloren, es fehlen Angaben. **Zu tun, in dieser Reihenfolge:** (1) **E-104 dokumentieren** — Kopfzeilen kleben nicht mehr zusammen (`compmsgd.cpp`, `UnwrapText` lief ueber die ganze Nachricht statt nur ueber den Rumpf) und `E104SchalterLesen` repariert verdorbene INI-Werte; es gibt dazu **weder einen BEFUNDE-Eintrag noch einen CHANGELOG-Abschnitt noch einen Test**. (2) **`EINSTELLUNGEN.md` und `FILTER.md`** nennen 7.2.0.67 nicht — `doku-pruefen.pl` weist deshalb ab. (3) **E-101 traegt in `BEFUNDE.md` die Fassung 7.2.0.66** — die P-28-Behebung steckt aber in **7.2.0.67**; die Zahl ist zu niedrig. (4) **Testzahlen**: an mehreren Stellen steht noch **160/160**, richtig sind **169/169**. (5) Der Arbeitsbaum `.claude/worktrees/relaxed-moser-c15950` (Zweig `p38-spurmarke-rumpf`) hat eine **nicht committete `msgutils.cpp`** aus einer Hintergrundaufgabe zu P-28 — **ueberholt**, P-28 ist ueber PR #59 in `main`; kann verworfen werden |
...
| **VON GREGOR ZU PRÜFEN, 17.09.2026 — E-101** | **die gespeicherte Datei ist jetzt für andere Programme brauchbar.** Gebaut in **7.2.0.67** (**169/169 Tests**, sieben davon neu), **am laufenden Programm von mir geprüft, von Gregor noch nicht bestätigt.** Prüfanleitung in [CHANGELOG.md](CHANGELOG.md), Abschnitt 7.2.0.67, *Zum Prüfen*: eine Nachricht über *File → Save As* sichern und die Datei in einem anderen Programm (Thunderbird, Editor) öffnen. **Was zu sehen sein muss:** `MIME-Version: 1.0` und `Content-Type: text/html; charset="ISO-8859-1"` in den Kopfzeilen, **kein** `<x-html>`, und die Umlaute stehen richtig da |
...
| **Zuletzt gebaut** | Quellstand **7.2.0.67** / `VERSION` **1.0.67**, gebaut am 17.09.2026 (**E-101**, 169/169 Tests). **Ein Paket dazu ist noch nicht geschnürt** — gemessen am 17.09.2026: unter `Releases/` liegt zuletzt das Verzeichnis `Eudora72-1.0.65-release`, das letzte ZIP ist `Eudora72-1.0.64-release.zip`. **Auf GitHub veröffentlicht ist weiterhin `v1.0.50`**; **1.0.51 bis 1.0.67 sind nicht freigegeben**. Aus Gregors Lauf mit **1.0.58** am 14.09.2026 stammen **E-89**, **E-90** (offen), **E-91** (offen) und **E-92** (offen); **E-95** meldete er an 1.0.60, **E-96** ist am 15.09.2026 an einem selbst gestarteten Eudora mit drei Bildern bekannter Größe gemessen worden. Bei 1.0.51 zeigte sein Test den Fehler E-85 noch, ab 1.0.52 ist E-85 behoben und belegt |
