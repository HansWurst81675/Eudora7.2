...
| 2 | Die Darstellung ist korrekt | **fast** — Fenster, Menüs und Werkzeugleiste stimmen (E-1, E-2), Bau-Kennung im Titel (E-7), Fortschritt beim Abruf (E-13), Umlaute **im lokal geschriebenen Text** (Z-2, Z-2b), „In" nur noch einmal unter *Recent* (E-24), gesperrte Knöpfe zeigen ihr Symbol (E-30), Doppelklick und Suchtreffer öffnen die Nachricht (E-28). **Offen sind zwei Punkte, ein dritter ist gebaut, aber von Gregor noch nicht am laufenden Programm geprüft:** die Meldung „Encountered an improper argument" beim Anzeigen mancher Nachrichten; **Umlaute in per IMAP abgerufenen Nachrichten** kommen falsch an (**E-85**, von Gregor am 13.09.2026 gemeldet; **in 7.2.0.51 behoben** — `ImapDownload.cpp` benutzt jetzt `FindMIMECharset` wie der POP3-Weg, der Rückgabewert von `ISOTranslate` wird ausgewertet, und `ISOTranslateChunk` trägt angefangene UTF-8-Zeichen über die Stückgrenze. **Am Quelltext belegt, von Gregor noch nicht am laufenden Programm bestätigt — und die Behebung hat eine Regression eingeführt, die noch offen ist:** die abschließende Null von `ISOTranslate` trifft im IMAP-Zeilenmodus das erste Byte der nächsten Zeile (`utils.cpp:1443`, Einzelheiten unter **E-85** in `BEFUNDE.md`). **Vor dem nächsten Paket zu schließen**); **IMAP-Postfachnamen mit Umlauten** stehen roh da, `Entw&APw-rfe` statt *Entwürfe* (**E-77**). Die Zeile hat bis zum 13.09.2026 schlicht „Umlaute" als erledigt geführt — Gregor hat den Widerspruch gefunden |
